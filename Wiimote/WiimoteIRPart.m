...
- (void)handle10ByteIRData:(const uint8_t*)data
{
	for (int i = 0; i < 4; i+=2) {
		uint32_t b2 = data[2];
		uint32_t x1 = data[0] | (((b2 & 0b00110000) >> 4) << 8);
		uint32_t y1 = data[1] | (((b2 & 0b11000000) >> 6) << 8);
		uint32_t x2 = data[3] | (((b2 & 0b00000011) >> 0) << 8);
		uint32_t y2 = data[4] | (((b2 & 0b00001100) >> 2) << 8);
		
		(x1 >= 1023 || y1 >= 1023) ? [self setPointOutOfView:i] : [self setPoint:i position:NSMakePoint(x1, y1)];
		(x2 >= 1023 || y2 >= 1023) ? [self setPointOutOfView:i+1] : [self setPoint:i+1 position:NSMakePoint(x2, y2)];
		data += 5;
	}
}
